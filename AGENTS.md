...
| GET | `/api/virtual-node` | Virtual companion node overview: listener state, counters, connected apps, remembered apps (history cursors), admin switch |
| DELETE | `/api/virtual-node/clients/{client_id}` | Forget a remembered app so its next connection starts at the present |
| POST | `/api/virtual-node/connections/{peer}/disconnect` | Close one connected app's socket |
...
| POST | `/api/settings/clock-sync-repeaters/toggle` | Toggle automatic clock sync for a tracked repeater during telemetry collection |
| POST | `/api/settings/clock-autofix-repeaters/toggle` | Toggle automatic forward-clock fixing (`clkreboot` + re-sync) for a clock-synced repeater |
| GET | `/api/settings/host-clock` | Whether this server's own clock is trusted to be pushed to repeaters (`?force=true` re-queries the time reference) |
...
| `MESHCORE_RESIDENT_CHANNELS_ENABLED` | `true` | Keep channels loaded in the radio's slots (Public first, then favorites and the most recently active, one scratch slot always kept free) so the firmware decrypts and queues their messages as a fallback for the raw RX-log route. `false` restores the original behaviour: every slot is wiped at startup and channels are loaded at send time only |
| `MESHCORE_RX_SILENCE_TIMEOUT_SECONDS` | `300` | RX silence watchdog: seconds without a raw RX-log frame before the radio's packet counters are probed. If the radio heard packets that never reached us, the link is reconnected and, failing that, the radio is rebooted. `0` disables the watchdog |
| `MESHCORE_TCP_KEEPALIVE_IDLE_SECONDS` | `30` | TCP keepalive idle time on a WiFi companion link so a dead peer is noticed within about a minute instead of hours; `0` leaves the OS default |
...
| `MESHCORE_HOST_CLOCK_NTP_SERVER` | `pool.ntp.org` | SNTP server used to verify this server's own clock before it is pushed to repeaters (`app/host_clock.py`); empty disables |
| `MESHCORE_HOST_CLOCK_HTTP_REFERENCE` | `https://www.cloudflare.com` | HTTPS URL whose `Date` header is the fallback time reference when NTP is blocked; empty disables |
| `MESHCORE_HOST_CLOCK_MAX_OFFSET_SECONDS` | `60` | Largest host-vs-reference offset still trusted; beyond it every clock push to repeaters is refused |
...
| `MESHCORE_VIRTUAL_NODE_REPLAY_LIMIT` | `1000` | Missed incoming messages replayed to a returning app (identified by APP_START name + peer address; cursor in `virtual_node_clients`). Newest win when more were missed; `0` disables replay |

**Note:** Runtime app settings are stored in the database (`app_settings` table), not environment variables. These include `max_radio_contacts`, `auto_decrypt_dm_on_advert`, `advert_interval`, `last_advert_time`, `last_message_times`, `flood_scope`, `known_regions`, `blocked_keys`, `blocked_names`, `discovery_blocked_types`, `tracked_telemetry_repeaters`, `tracked_telemetry_contacts`, `clock_sync_repeaters`, `clock_autofix_repeaters`, `auto_resend_channel`, `telemetry_interval_hours`, and `virtual_node_allow_admin_commands` (default off: whether apps on the virtual companion node may change radio settings). `max_radio_contacts` is the configured radio contact capacity baseline used by background maintenance: favorites reload first, non-favorite fill targets about 80% of that value, and full offload/reload triggers around 95% occupancy. They are configured via `GET/PATCH /api/settings`. MQTT, bot, webhook, Apprise, and SQS configs are stored in the `fanout_configs` table, managed via `/api/fanout`. If the radio's channel slots appear unstable or another client is mutating them underneath this app, operators can force the old always-reconfigure send path with `MESHCORE_FORCE_CHANNEL_SLOT_RECONFIGURE=true`.
