...
- `GET /radio/channel-slots` reads every slot from the radio under the operation lock and annotates each with residency, send-cache membership and whether RemoteTerm has the key (`known_name`); it is a button in the radio settings, not a poll. `POST /radio/cli` runs one firmware CLI command through `services/radio_cli.py`: the pinned meshcore-py (2.3.7) predates `CMD_RUN_CLI_COMMAND` (66) and `RESP_CODE_CLI_REPLY` (29), so the frame is built by hand, the reply is caught by a `reader.handle_rx` wrap installed with the other adapters in `register_event_handlers`, and the firmware gate is companion protocol 14 (`radio_manager.firmware_ver_code`, the same check meshcore-cli uses). One command in flight at a time -- the reply carries no correlation id -- which the operation lock guarantees. 501 when the firmware is too old, 502 on an error frame, 504 on silence.
...
- **Diagnosing a miss.** The hash is one-way, so "matched nothing" alone says nothing about *why* two clients disagree. `_log_unmatched_reaction` logs the reaction hash at WARNING and, at INFO, what we hashed for the newest few eligible messages (id, timestamp, sender, first 5 code units). Zero eligible candidates means the direction rule or the conversation key excluded the target before hashing.
- **`_probe_reaction_mismatch` names the fault.** The hash cannot be inverted, but the ways two clients disagree about one message are few, and each is a rule to re-hash under: a timestamp offset (±15 min second by second, plus whole-hour offsets for a peer working in local time), the sender-name rule we did *not* use (the two ends disagreeing about whether the conversation is a room), and body normalisations (surrounding whitespace, a trailing NUL off the companion frame, a stripped reply mention). The first hit is logged as "it would have matched msg 42 ...", which is the actual fault rather than a hash to eyeball. **Diagnosis only** -- none of it is accepted as a match, because a 16-bit hash tried under thousands of variants starts colliding, and a reaction on the wrong message is worse than one with no message. Bounded to the newest 10 candidates, ~85 ms worst case, and only on the failure path.
- **The hash vectors are real Dart.** `tests/fixtures/reaction_hash_vectors.json` is generated by running `ReactionHelper.computeReactionHash` and `String.hashCode` on the Dart VM (SDK 3.13.3), not by transcribing `runtime/vm/hash.h`. The 33 vectors the transcription produced all reproduced exactly, so the port is confirmed against the runtime MCO Advanced actually runs on -- worth knowing before suspecting the hash when a reaction misses: it is the inputs, not the algorithm.
