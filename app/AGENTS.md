...
│   ├── radio_cli.py             # Companion CLI over the host link (CMD 66 / RESP 29), reply caught by a reader tap
│   ├── rx_watchdog.py           # RX silence watchdog: packet-counter probe, reconnect, reboot
...
5. Channel messages have the same two routes: the raw RX-log frame decrypted by `packet_processor._process_group_text` (primary, carries path/SNR/RSSI) and, for channels resident in a radio slot, the firmware-decrypted copy pulled from the radio's queue and stored by `event_handlers.on_channel_message` -> `radio_sync.store_pulled_channel_message` (fallback, no path). Both land through the `CHAN` dedup index, so the second arrival collapses onto the first. The drain/poll loops store what they pull themselves and hold `radio_sync.inline_message_pull()` while doing so; the handler checks `is_inline_pull_active()` before its first `await` and stands down. The handler runs outside any radio operation, so when a pulled message names a non-resident (scratch) slot its `get_channel` lookup takes the operation lock first: meshcore-py neither serializes commands nor tags replies, and a concurrent slot scan would otherwise steal its `CHANNEL_INFO`. Resident slots are answered from memory. The periodic full sync suspends auto-fetch for the same reason.
...
- `services/rx_watchdog.py` runs alongside the monitor. The link has no signal for "connected but the radio stopped pushing RX-log frames" (commands still answer, the socket stays open), and in the field that failure looked exactly like a quiet mesh: DMs kept arriving through the firmware queue while every Public message was lost. The watchdog measures silence from `RadioManager.note_rx_log_frame()` (fed by `on_rx_log_data`) or from `mark_rx_baseline()` at setup completion, and after `MESHCORE_RX_SILENCE_TIMEOUT_SECONDS` probes `STATS_PACKETS`. Only a `recv` counter that grew across a window with no frame proves the push path is wedged; then it reconnects, and on the next confirmed window sends `reboot` (at most once per 30 min). A firmware without packet stats gets probes and a warning, never an escalation. The probe itself is a command exchange, which is what un-wedged the link when this was observed.
- The watchdog never toasts. Lulls, recoveries, the reconnect and the reboot are all log lines only; the connection indicator the UI already shows reflects the reconnect, and a toast would duplicate it.
- `GET /radio/channel-slots` reads every slot from the radio under the operation lock and annotates each with residency, send-cache membership and whether RemoteTerm has the key (`known_name`); it is a button in the radio settings, not a poll. `POST /radio/cli` runs one firmware CLI command through `services/radio_cli.py`: the pinned meshcore-py (2.3.7) predates `CMD_RUN_CLI_COMMAND` (66) and `RESP_CODE_CLI_REPLY` (29), so the frame is built by hand and the reply is caught by a `reader.handle_rx` wrap installed with the other adapters in `register_event_handlers`. One command in flight at a time -- the reply carries no correlation id -- which the operation lock guarantees. **Most radios cannot do this at all**: the MeshCore CLI belongs to repeater and room-server firmware, and released companion firmware implements commands only up to 65, so it answers 66 with `ERR_CODE_UNSUPPORTED_CMD` (error code 1, in its `handleCmdFrame` final else). That error code is the authoritative signal and maps to `CliUnsupportedError` -> 501; it is latched on `radio_manager.cli_unsupported` for the connection (cleared in `_reset_connected_runtime_state`, same pattern as `raw_data_unsupported`), so a second attempt is refused without a radio round trip and `GET /radio/config` reports it as `cli_unsupported` for the settings panel to disable its input. The companion-protocol-14 check is only a pre-check that spares a round trip -- passing it does not mean the command exists. Any other error code is a genuinely failed command (502); silence is 504.
- `RadioManager.apply_tcp_keepalive()` sets `SO_KEEPALIVE` (idle `MESHCORE_TCP_KEEPALIVE_IDLE_SECONDS`, 3 probes at a third of that) on the socket meshcore-py opened. It runs at connect and again at every post-connect setup, because the library replaces the transport when it reconnects on its own.
...
`app/virtual_node/` is an optional TCP server (`MESHCORE_VIRTUAL_NODE_ENABLED`, default port 5000) that speaks the MeshCore companion protocol so other apps use this server as their radio. It answers identity, contacts, channels, clock, battery and the inbound message queue from server state, caches read-only device queries for 30 s, and forwards everything else to the radio under `radio_operation`. App sends go through `services/message_send.py` like web sends, so they are stored, broadcast and ACK-tracked. `CMD_SEND_CHANNEL_DATA` (62) — every picture MCO Advanced sends, and every compressed channel message while its binary setting is on — is handled locally so the app's virtual slot is translated to the radio's, and GRP_DATA blobs are relayed to apps in both directions (`mirror_channel_data`). `websocket.broadcast_event` mirrors realtime `message`/`contact`/`channel` events into it. Returning apps (identified by APP_START name + peer address, cursor in `virtual_node_clients`, migration 084) are replayed the incoming messages they missed up to `MESHCORE_VIRTUAL_NODE_REPLAY_LIMIT`. Read `app/virtual_node/AGENTS_virtual_node.md` before changing the command policy.
...
- Resident channels (`MESHCORE_RESIDENT_CHANNELS_ENABLED`, default on): `radio_sync.sync_and_offload_channels` still imports every slot into the DB, but then writes the resident set back -- `select_resident_channels`: Public, then favorites, then most recent `received_at`, then name, capped at `max_channels - 1` so one scratch slot stays free -- and clears only the rest. A slot already holding the right channel is not rewritten. `RadioManager` tracks them separately from the send cache (`set_resident_channels`, `get_resident_channel_slot`, `channel_key_for_slot` consults residents first); `channels.on_radio` mirrors the set. Joining a channel pins it into a free slot best-effort (`add_resident_channel_to_radio`, from the router, in a background task that waits for the radio lock, never blocking the response); deleting one drops the in-memory mapping synchronously (so a queued message can no longer resolve to it) and clears the slot in the background. A queued message whose resident slot maps to a channel no longer in the DB is dropped, never used to re-create the channel. The poll-loop audit (`audit_resident_channels`) re-pins any resident slot another client overwrote and clears the slot of a channel deleted while the radio was unreachable; it checks every slot on the hourly tick and one slot per tick (round-robin) in aggressive 10-second poll mode, so it never hogs the link. `routers/debug.py`'s channel audit counts resident slots as expected. Off, the original wipe-everything behaviour applies.
- Channel sends: a resident channel always sends from its pinned slot (no `set_channel` on reuse transports; on TCP the same key is rewritten into the same slot). Non-resident channels use a session-local LRU cache over the remaining scratch slots: repeated sends reuse the loaded slot, new channels fill free scratch slots, then evict the least recently used cached channel -- never a resident one.
...
- The catalog is deliberately a **superset** of any one firmware build. An unknown key is answered by the generic config handler with `"??: <key>"` on `get` and `"unknown config: <key>"` on `set`, never with silence; a key the build knows but the board cannot honour (FEM gain, nRF52 power management, the battery ADC multiplier) answers `"Error: unsupported"` / `"... not supported"`. `is_unsupported_reply` treats all of those as `unsupported` so the UI locks the field. Do not add firmware-version gates per key — a key that stops existing degrades on its own. The catalog mirrors `handleGetCmd`/`handleSetCmd` in the firmware's `src/helpers/CommonCLI.cpp`; when adding a key, copy its bounds and its spelling quirks from there (`rxdelay` but `direct.txdelay`; `bridge.source` reads back `logRx`/`logTx` but is written `rx`/`tx`, hence `reply_aliases`).
- **Not every key is `set <key> <value>` with `on`/`off`.** The admin password is a bare top-level `password <new>` command (`set_verb=None`; `set password` is answered "unknown config"), and `multi.acks` is parsed with `atoi`, so it must be written `1`/`0` (`bool_style="digit"`) — the word `on` reads as zero and turns the feature off. `command_hint` is what the UI shows beside the field, so a key with an odd command shape still reads correctly there. `prv.key` and `set freq` are deliberately absent: the firmware only honours them from the serial console.
...
- Password values (`password`, `guest.password`, `bridge.secret`) are `sensitive`: they go out on the wire but are redacted out of the apply response and the server log, and the admin password is `readable=False` because the firmware cannot return it. The firmware echoes a new admin password back in its reply (`password now: ...`), so the apply route redacts the value out of a sensitive setting's reply too.
...
- **Over GRP_DATA, not only as text:** MCO Advanced's `channelsSendAsBinary` setting is **on by default**, so a compressed *channel* message from a current build arrives as a binary GRP_DATA blob rather than as `mcmp2:`/`mcmp3:` text — legacy type `0xFFF1` (v2 body) or app type `0x0120` subtype 2 (the v3 container). `app/imaging/aeic/channel_data_text.py` unwraps the envelope and hands the body to the same codec; `channel_data_ingest.handle_channel_data` then stores it through `create_fallback_channel_message`, so both delivery paths (frame 27 and raw RF) and both wire forms produce one ordinary channel message. The decoder is not self-checking — fed anything it returns *something* — so a decoded body that is not plausible prose (empty, control characters, or the decoder's 4096-char hard limit) is refused and the blob is treated as unsupported media, as before.
...
- **The hash is Dart-specific:** low 16 bits of Dart VM `String.hashCode` over `"{senderTimestampSecs}{senderName}{first5}"` (channels/rooms) or without the sender (1:1 DMs), where `first5` is the first 5 **UTF-16 code units** of the body. The Python port is validated against `tests/fixtures/reaction_hash_vectors.json`, generated from a C transcription of the Dart SDK reference (`runtime/vm/hash.h`, finalized to 30 bits, 0→1). Do not "fix" the odd bits (UTF-16 slicing that can split a surrogate pair, the 30-bit mask) — they are the interop contract.
- **Hash inputs come from MCO Advanced's view of the message**, not ours: channel bodies are re-split with its `_splitSenderText` port (`split_channel_sender_text`; sender falls back to `"Unknown"`, and a bare `r:...` on a channel splits as sender `"r"` — a nameless node cannot emit channel reactions, same blind spot upstream) and reply mentions/exact-quote lines are stripped first (`clean_channel_body_for_hash`). Room posts hash with the author's contact name (ours: `sender_name`; outgoing: the radio name); 1:1 DMs hash with no sender.
- **Ingest:** every message-creation seam (`create_message_from_decrypted`, `create_fallback_channel_message`, `_store_direct_message`, and the outgoing creators) detects reaction payloads. The row is stored like any message so echo/repeat dedup keeps working, but flagged `is_reaction` (migration 082) and hidden from every conversation surface — list/around/search, unread counts, last-message times, `message` broadcasts (so fanout/bots/push never see it). Its effect lands on the target row's `reactions` JSON (`{emoji: count}`), applied newest-first over the last 500 messages with MCO Advanced's direction rules (1:1: incoming reactions match only our outgoing messages and vice versa), then broadcast as `message_reaction`. New identical rows count again (two reactors = 2); duplicate inserts (flood echoes, retries) reconcile as echoes and never re-apply.
- **An unmatched reaction stays visible.** When the scan finds no target the row is unhidden (`unhide_unmatched_reaction`) and broadcast as an ordinary message, so the chat renders it generically ("👍 reacted") instead of showing nothing at all. MCO Advanced drops such a reaction because it only has an in-memory window to match against; we have the row, and losing the fact that a peer reacted is worse than an unattributed bubble. That broadcast is deliberately **non-realtime**, so fanout, bots and push never see a raw `r:HHHH:II` payload. Rows the migration backfilled stay hidden -- they were never applied and re-showing months of history would be its own regression.
- **Diagnosing a miss.** The hash is one-way, so "matched nothing" alone says nothing about *why* two clients disagree. `_log_unmatched_reaction` logs the reaction hash at WARNING and, at INFO, what we hashed for the newest few eligible messages (id, timestamp, sender, first 5 code units). A right body under a different timestamp is a clock/echo problem; a matching timestamp under a different body is a text-normalisation one; zero eligible candidates means the direction rule or the conversation key excluded the target before hashing.
- **Sending:** `POST /api/messages/{id}/react` validates the emoji against the table, computes the hash from the stored target, and pushes `r:HHHH:II` through the normal channel/DM send orchestration (slots, scope overrides, retries). Reactions are never MCMP-wrapped (`is_framed_payload` counts them as protocol, matching MCO Advanced sending them plain). The reaction is applied locally only after the radio accepts the send; the scan-first/fallback-target logic in `apply_reaction` keeps hash collisions landing on the same (newest) message every client picks.
- In 1:1 DMs you cannot react to your own messages — the peer matches incoming reactions against **their outgoing** rows only, so such a reaction could never land. The endpoint enforces this (400).
...
- Firmware without `CMD_SEND_RAW_DATA` raises `RawDataUnsupportedError` (→ HTTP 501) only when the text transport below is unavailable. **MCO Advanced never sends raw data at all**, so this transport has no reference implementation to interoperate with.
- **What actually uses the command.** Sending a picture does not: `POST /images/send` stores the fragments locally and puts only the `IE4:` envelope on the air as an ordinary message. Opening your *own* picture does not either — `fetch_image` skips the radio when the fragments are already local. Nor does AEIC, which is text end to end. The command is used in exactly one case: **opening a received `IE4:`/`VE3:` message**, for the fetch request out and the fragments back. Worth knowing before debugging "images sometimes work".

### Text transport for raw media (`rmt1:`)

- `app/services/raw_media_text.py` carries the *same payload bytes* the raw transport would have, basE91-encoded in ordinary direct messages, and hands them to the same `dispatch_raw_media_payload`. Nothing above or below knows which transport ran. Same trick as `app/imaging/aeic/text_transport.py`, same reason: a text message is the one transport every route already handles.
- Wire format is a fixed-width header, `rmt1:<sid:2><idx:1><tot:1>` then payload — no delimiter parsing, because the basE91 alphabet contains `:`. 9 characters of header, 147 of payload per message, 36 chunks max (~4.3 KB, far above the largest real payload).
- **It costs ~2.5x the airtime.** A 158-byte image fragment becomes 195 basE91 characters, which does not fit one 156-byte message — so every fragment is *two* messages, and a 20-fragment picture is 40 messages and minutes of airtime. `test_a_full_image_fragment_costs_two_messages` pins that arithmetic. It is the default anyway: a node that *can* send raw data loses only airtime by tunnelling, a node that cannot gets a picture it could not otherwise open, and there is no way to tell which kind a peer is before asking.
- **DMs only, never stored.** The raw transport is contact-directed even for a picture announced on a channel (both fetch handlers resolve a contact), so the tunnel matches it — which also keeps a 40-message transfer off a shared channel. Chunks are sent below the message layer (no row, no bubble, no ACK tracking) and dropped in `_store_direct_message` before storage, dedup and broadcast. Our own echoed chunks are dropped *without* being processed, or we would answer our own fetch request.
- **Two rules pick the transport, and only two** (`_resolve_transport`). What we *initiate* follows the switch, with no raw attempt first — on the firmware this exists for, a first attempt is both doomed and the only way to find out. What we *reply* mirrors the transport its request arrived on, threaded down as `MediaTransport` from `dispatch_raw_media_payload`. Mirroring is not a preference: a raw request answered over text is unreadable to the SAR client that sent it, and a text request answered raw leaves our radio and arrives nowhere. It is also what keeps MCO Advanced working with the switch on.
- The arriving packet states its own transport, so nothing is remembered. An earlier version kept a TTL map of "peers seen speaking the tunnel" (`note_text_tunnel_peer`) to decide the same thing — a second source of truth for a fact the request already carries, and one that guessed where mirroring knows.
- `radio_manager.raw_data_unsupported` (per connection, cleared in `_reset_connected_runtime_state`) stops each of 40 fragments spending a doomed round trip rediscovering the firmware limit. Now only reachable while mirroring raw requests, since a request we start never tries raw with the switch on.
- Voice fragment ACKs are **skipped** when the fragment arrived over text: nothing consumes a received ACK here, and a text peer is necessarily RemoteTerm, so it would be one extra message per fragment for no reader.
- The switch is `contacts.raw_media_text_transport` (migration 077 added it as `..._fallback`, 078 renamed it when it stopped being one; default on), surfaced in the conversation features modal. **Contacts only, no channels column** — a channel one would never be read, and dead UI is worse than an absent toggle.
- **Recovery lives one layer up, and only there.** The tunnel carries and reassembles bytes; it has no parity, checksum or chunk retransmission. A lost chunk loses its payload, which is one *fragment* — exactly what `missing_indices` already re-requests, over the tunnel, at the granularity that matters. A chunk-level NACK would be a second retry loop under a working one. The price is that a lost chunk wastes its sibling: two messages re-sent to replace either one. The gap it leaves is a lost fetch *request*, which no fragment count can detect — the person taps again.
- Both session payloads report `transport`, so the client can pace its polling. It is what *we* would use to ask (and replies mirror us), so it is right for every fetch this app starts and says nothing about an inbound request.
- A peer without this feature sees the raw `rmt1:` lines as chat. Ugly, harmless, and the honest signal that it cannot join in. It does not lock such a peer out of *its own* fetches, which arrive raw and are answered raw.
...
- **Scope**: both sides of the merge are restricted to the compared channels (a deselected channel's rows stay mirrored but never show as live-only). Rows do not record the region they were fetched under, so a change of instance URL or region clears the mirror and walks again; a change of channel selection only forces a full walk. `LiveFeedState.recent_log` keeps the last 40 timestamped sync events (start, slice failures, fallbacks, summary, errors) and is exposed as `recent_log` on the status for the settings page's activity view.
...
- A tracked repeater may also opt into `app_settings.clock_sync_repeaters`. When set, each telemetry cycle that reaches the repeater also sends the CLI `time <epoch>` command (same as the manual "Sync Clock" action) via `_sync_repeater_clock`, which awaits and parses the CLI reply (`fetch_contact_cli_response`). The firmware only ever moves a repeater clock *forward*: a repeater already ahead of the server answers `(ERR: clock cannot go backwards)`, so the helper then fetches the repeater's `clock` (parsed by `clock_drift.parse_firmware_clock`) and logs the offset at WARNING instead of claiming success — only a reboot resets a volatile clock, and a server whose own clock was wrong when a sync ran will have pushed that wrong time permanently. Everything is best-effort — no reply (radio not logged in as admin), send errors, or exceptions are logged and never fail telemetry collection or recording. This list is a subset of `tracked_telemetry_repeaters`; removing a repeater from telemetry tracking cascade-clears its clock-sync flag too (`routers/settings.py`).
- A clock-synced repeater may further opt into `app_settings.clock_autofix_repeaters` (migration 087). When its sync is refused because it is ahead by at least `AUTOFIX_MIN_AHEAD_SECONDS` (120 s), `_maybe_autofix_clock` runs `fix_forward_clock` at most once per `AUTOFIX_COOLDOWN_SECONDS` (24 h) per repeater: CLI `clkreboot` (the firmware sets its clock to 15 May 2024 and reboots), a `FIX_CLOCK_REBOOT_WAIT_SECONDS` wait, then `_sync_repeater_clock` again. The repeater persists its admin ACL across reboots, so no re-login is normally needed; the manual endpoint can pass the dashboard's last password for one re-login if the repeater does not answer. `fix_forward_clock` refuses to reboot a repeater that does not read ahead (`not_ahead`, plain sync instead). The list is a subset of `clock_sync_repeaters` and is cascade-cleared with it.
- **Every clock push is gated on `app/host_clock.py`.** The firmware ratchet makes a push from a wrong server clock permanent, so `check_host_clock()` (cached 10 min) first asks an external reference -- SNTP against `MESHCORE_HOST_CLOCK_NTP_SERVER`, then the `Date` header of `MESHCORE_HOST_CLOCK_HTTP_REFERENCE` -- and reports `trusted=False` when the host is more than `MESHCORE_HOST_CLOCK_MAX_OFFSET_SECONDS` off. With no reference reachable the clock is *unverified* and pushes proceed, except after the wall clock visibly steps against the monotonic clock (a VM clock jump seen from inside), which holds pushes for `STEP_QUARANTINE_SECONDS`. The telemetry cycle checks once per cycle and skips all syncs/auto-fixes when untrusted; `POST /contacts/{key}/repeater/sync-clock` and `.../fix-clock` return `server_clock_untrusted`; `GET /settings/host-clock` exposes the verdict.
...
- `POST /settings/clock-autofix-repeaters/toggle` — toggle automatic forward-clock fixing (`clkreboot` then re-sync, see `radio_sync.fix_forward_clock`) for a repeater; requires `clock_sync_repeaters` membership, cascade-cleared with it
- `GET /settings/host-clock` — `HostClockStatus`: whether this server's own clock is trusted to be pushed to repeaters (`app/host_clock.py`); `?force=true` bypasses the 10-minute cache
- `POST /contacts/{key}/repeater/sync-clock` — push the *server's* clock to a repeater and return the firmware's verdict (`set`/`ahead`/`no_reply`/...); `server_clock_untrusted` when the host-clock check fails
- `POST /contacts/{key}/repeater/fix-clock` — `clkreboot` + wait + re-sync for a repeater whose clock is ahead (`not_ahead` refuses to reboot a healthy one); optional `password` for one re-login; holds the radio ~30 s
...
├── test_virtual_node.py        # Virtual companion node: frame codecs (round-tripped through meshcore's reader), the TCP proxy server, and GRP_DATA in both directions
