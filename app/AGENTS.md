...
- Firmware without `CMD_SEND_RAW_DATA` raises `RawDataUnsupportedError` (→ HTTP 501) only when the text fallback below is unavailable. **MCO Advanced never sends raw data at all**, so this transport has no reference implementation to interoperate with.
...
### Text fallback for raw media (`rmt1:`)
...
- **Two learned facts make it usable.** `radio_manager.raw_data_unsupported` (per connection, cleared in `_reset_connected_runtime_state`) stops each of 40 fragments spending a doomed round trip rediscovering the firmware limit. `note_text_tunnel_peer` remembers a peer that tunnelled to us, so the reply goes back the same way — firmware without the send command does not push received raw data up either, so raw fragments would leave our radio and arrive nowhere.
- Voice fragment ACKs are **skipped** for tunnel peers: nothing consumes a received ACK here, and a peer on the tunnel is necessarily RemoteTerm, so it would be one extra message per fragment for no reader.
- The switch is `contacts.raw_media_text_fallback` (migration 077, default on), surfaced in the conversation features modal. **Contacts only, no channels column** — a channel one would never be read, and dead UI is worse than an absent toggle.
- A peer without this feature sees the raw `rmt1:` lines as chat. Ugly, harmless, and the honest signal that it cannot join in.
