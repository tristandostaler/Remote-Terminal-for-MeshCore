...
│   │   ├── RadioCliConsole.tsx           # Collapsible under Connection: one-line console to the radio firmware CLI, with history; a 501 (or `config.cli_unsupported`) replaces the input with "this firmware has no CLI"
...
- **A whole-catalog read is chunked client-side** (`planSettingsReads` in the hook): the catalog's readable keys, minus `excludeKeys`, go out as `{ keys }` requests of `SETTINGS_READ_CHUNK_SIZE` (5) each, in order. One request per setting-round-trip × fifty settings outlives the 60-second timeout of Home Assistant ingress / nginx and comes back as a 504, so never issue the whole catalog as one request. Chunks fill the editor as they land; the loop stops after the first chunk in which nothing at all answered (a guest session), since every further chunk would cost the same silence. A `{ group }` or `{ keys }` read is sent as-is.
- **A value shown in two places is read once.** Node Info shows name/lat/lon and Owner Info shows the owner note and guest password; the editor has fields for all five. Those pane endpoints return the same replies as `settings` entries, which `refreshPane` merges into `settingsValues`, and "Load All" passes the keys they covered as `excludeKeys` to the settings read. The other way round, `fetchSettings`/`applySettings` copy confirmed values onto a pane that already has data (`mirrorSettingsIntoPanes`; a never-fetched pane is left alone rather than shown half-filled). The mapping lives in `PANE_SETTING_FIELDS` in the hook and must match what the backend endpoints emit.
...
- **Status drives the field, not a firmware version check.** `unsupported` (the firmware answered `??`, `unknown config` or `Error: unsupported` — a key this build or this board does not have) locks the field; `no_reply` across the board sets `cli_responsive: false`, which is what a guest session looks like and is shown as "log in as admin". The Bridge group and the FEM gain switches lock themselves this way on most hardware; that is expected, not a bug.
- **The command shown beside a field is `command_hint`, not `set ${cli_key}`.** The admin password is a bare `password` command and the Device Info group is read-only `get`s, so the hint comes from the catalog rather than being assembled here.
...
- `LiveCompareView.tsx` owns one window (`WindowSelector` from `nodeStatsShared.tsx`), a source filter with counts, a channel filter (only when more than one channel is compared) and a text filter. Two requests: `api.getLiveCompareStats` (status + tiles) and `api.getLiveCompareMessages` (paged list). Only the cheap status endpoint is re-polled every 30 s; a newer `last_sync_completed_at` reloads the stats and the list, so the page follows the backend sync loop without WebSocket plumbing and without re-running the comparison query on a timer. `seen_at` (earliest of node receive / live first observation) is the sort key.
- Empty state when the feature is off points at Settings › Live Compare via `onOpenSettings` (App's `onOpenLiveFeedSettings`).
- Each row's hops/observers line is a button (`live-compare-inspect`) opening `LiveCompareTraceDialog` (lazy), which fetches `api.getLiveCompareTrace({packetHash, messageId})` — the hash for live/both rows (the row `key`), the id for node rows — and shows a one-paragraph reading (`readTrace` in `liveTraceShared.ts`: did a direct neighbour repeat it, how far the closest located relay was, whether any relay is in the contacts), a Leaflet map of every route (`LiveTraceMap.tsx`, one marker per node however many routes it sits on, dashed lines where a hop has no location) and one card per reception with hop chips (sky = in your contacts, plain = named by the instance, dashed = unknown, `?` = ambiguous; the `title` carries the alternatives and the distance from this node). Colours per role live in `liveTraceShared.ts`. Tests mock `react-leaflet`.
...
- `settings/SettingsLiveFeedSection.tsx` (section key `live-feed`) edits the five `live_feed_*` settings. Channels are a checklist of every channel this node holds a key for (Public always listed; hashtag/private badged) behind an "All channels" switch that stores `['*']`; an explicit selection stores channel keys. The region picker is filled from `api.getLiveFeedRegions()` (the instance's own list, IATA codes) with a free-text fallback for several codes; a status box shows the last sync, which channels are compared, whether the sync decrypted remote packets locally (`source: 'packets'`) or fell back to the instance's own decryption, and a Sync now button.
