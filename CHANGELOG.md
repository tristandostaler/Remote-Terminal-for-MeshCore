...
* Feature: The repeater settings editor now covers every setting the firmware's `get`/`set` CLI exposes, not just the two dozen it started with. Radio gains **RX Boosted Gain** (`radio.rxgain`), the two front-end-module switches (`radio.fem.rxgain`, `radio.fem.txgain`), **Channel Activity Detection** and the separate hop limits for adverts and unscoped floods; Advanced gains the interference threshold, path hash mode, loop detection, the battery ADC multiplier and the LR2021 extra spreading factors; a new **Bridge** group holds the RS232 / ESP-NOW bridge settings; and a read-only **Device Info** group reads back the role, public key, bootloader version, power source, last boot reason and voltage at boot. As before, a field the firmware or the board does not have locks itself rather than offering an edit that cannot work -- and that now also covers the firmware's "Error: unsupported" answer, which a board without a front-end module or nRF52 power management gives for a key the build does know, and the "unknown config" answer a `set` gives for a key it has never heard of; both used to show as a plain firmware error. Ranges now match what the firmware actually accepts: the delay factors are 0 to 2 and the RX delay base 0 to 20 rather than 10,000, the flood advert interval runs to a week, the duty cycle starts at 1%, a name may not contain the characters the firmware refuses, and the radio tuple accepts the 2.4 GHz band LR2021 boards run on

* Bug: "Load All" and "Read All" on a repeater dashboard could come back as a bare "504 Gateway Time-out" page from nginx when RemoteTerm sits behind Home Assistant ingress or another reverse proxy. Every setting is a round trip over the air -- a second to settle the radio and up to ten seconds waiting for the reply -- and the whole catalog went out as one HTTP request that answered only once the last setting was in. With the catalog now near fifty settings, that request outlived the proxy's sixty-second limit, so the proxy gave up on it even though the server was still happily reading. The whole-catalog read now goes out five settings at a time, each request well under a minute even on a slow mesh, and each fills the editor as it lands instead of everything appearing at the end. A session that is not an admin gets no reply to anything, so the read stops after the first handful rather than sitting through the same silence for every chunk

* Bug: The **Duty Cycle Limit** field in the repeater settings editor showed nothing after a read, although the console answered `get dutycycle` with "100.0%" and the value was right there in the page's HTML. The firmware appends the percent sign for display, the editor puts that setting in a numeric input, and a browser renders a numeric input blank for anything that is not a bare number. A numeric setting answered with a unit attached is now reduced to the number before it reaches the form, so the field reads 100.0 and can be edited and written back; a reply that is not a number with a unit is still shown as-is rather than lost, and read-only facts such as the boot voltage keep their unit

* Bug: Two fields in the repeater settings editor never worked. Changing the **Admin Password** sent `set password ...`, and the firmware has no such setting -- it takes the new password as a bare `password` command -- so every attempt was answered "unknown config" and the password stayed as it was. It now sends the command the firmware expects, and the firmware's echo of the new password in its reply is redacted from the result and the console history the way the value itself already was. **Multi ACKs** sent `on`/`off`, and the firmware reads that key as a number, so `on` read as zero and switched the feature *off* whichever way it was set; it now sends `1`/`0`

* Feature: Undecrypted packets can now be re-tried against the keys you have, from the UI, without deleting and re-adding anything. Until now the only moment a stored packet was ever re-tried was the "try decrypting N stored packets" checkbox at the instant a room or contact was added -- off by default, easy to miss, and gone forever once the room existed. Settings → Database gains **Retry Decryption With All Keys**, which walks every packet that never decrypted and tries every channel key you have against each one; a room's info pane and a contact's info pane each gain a **Retry historical decrypt** of their own for a single conversation. The DM sweep no longer needs the node's private key in the browser -- the button sends only the contact's public key and the server uses the key the radio exported. All the keys are tried in one pass over the packets, so sweeping fifty rooms costs one scan rather than fifty, and sweeps now queue behind each other instead of running several full scans of the same table at once
* Feature: A running decrypt sweep now reports itself: a progress bar with "24,000 / 91,000 packets", how many messages it has recovered so far, and which conversation each one landed in ("#ops -- 12 messages"). It appears in Settings → Database and in the info pane of whatever is being swept, updates a few times a second rather than on every packet, and survives a page reload or a second browser tab, which pick up a sweep already in progress
* Feature: Messages recovered by a decrypt sweep are now findable instead of being silently filed into the past. They keep the time they were actually heard, so they sit in the conversation where they belong, and each is badged **Recovered** with the date it was decrypted. Because unread state is measured against when you last read a conversation, a message recovered from a three-week-old packet would otherwise land below that mark and never be shown -- so the conversation is now marked unread for it, the unread divider anchors at the oldest recovered message, and reading the conversation clears it as usual. Conversations do not jump around the sidebar while a sweep runs: recovering an old message does not change when you last heard from someone

* Bug: An emoji reaction sent from MeshCore Open Advanced could vanish on arrival -- the packet appears in the packet feed, and the conversation shows nothing at all. A reaction does not name the message it belongs to: it carries a four-character hash of that message's timestamp and first five characters, and each client matches that hash against its own copy. When the two copies disagree about either -- a timestamp the other end recorded differently, a body one side stored in another form -- nothing matches, and the reaction was then hidden on the grounds that it had already been applied somewhere, so the only trace of it was a line in the log. A reaction that matches nothing is now kept and shown as a plain "👍 reacted" bubble in the conversation instead: it cannot say which message it belonged to, but knowing that someone reacted beats showing nothing. It is not passed on to bots, MQTT or push notifications, which have no use for a raw reaction payload. The log also now says *why* a reaction matched nothing, listing what was hashed for the most recent messages it was compared against -- the timestamp, the sender and the first five characters -- so a mismatch that comes from the clock and one that comes from the text are told apart at a glance instead of both reading as "matched no recent message"

* Bug: The "Direct TX Delay Factor" field in the repeater settings editor said "not supported by this firmware" on every repeater, and could not be read or changed. The firmware calls that setting `direct.txdelay`, and the editor was asking for `direct.tx.delay`, which no firmware knows and which every firmware answers with the "unknown setting" reply the editor takes as its cue to lock the field. It now asks by the right name, so the field reads and writes like the two delay settings beside it

* Feature: A repeater's settings can now be changed from its dashboard, without typing a single CLI command. Logging in as admin adds a Settings Editor below the panes covering everything the firmware exposes: its name, position and owner note; the radio itself -- frequency, bandwidth, spreading factor, coding rate, transmit power, airtime factor, duty-cycle limit, repeat mode and maximum flood hops; both advert intervals; the admin and guest passwords and whether non-admins may read anything; the three telemetry modes; and the timing knobs under Advanced. Each group is read on demand rather than all at once, because every setting costs its own round trip over the air and reading the lot holds the radio for minutes; then it is edited as a form and written back in one go, behind a two-click confirm, one command per field you actually changed. What the repeater answered for each field is shown against it and mirrored into the console alongside anything typed there, so a write that was refused is visible instead of silent, and a field that did not take keeps its old value and stays marked as changed, so one more click retries exactly those. A setting this firmware does not have answers "??" rather than staying quiet, so that field says "not supported by this firmware" and locks itself instead of offering an edit that could never work -- one editor covers old and new firmware without guessing from a version string. A session that is not an admin gets no reply to anything at all, since the firmware routes no CLI text for a guest, and the pane now says so plainly rather than showing an empty form. Values are checked before anything reaches the air: a number out of range, a radio tuple missing a part, or a name with a line break in it -- which would be a second command smuggled onto the repeater -- is refused, and one bad field stops the whole batch instead of half-applying it. Passwords are masked, never read back into the form, and redacted everywhere the write is recorded. The read-only Radio Settings pane is gone with it: everything it showed -- the radio tuple, transmit power, airtime factor, duty cycle, repeat mode, flood hops and both advert intervals -- is now shown and edited in the one place, rather than in a display copy that could disagree with the form beside it, and the firmware version it also carried is already in Owner Info (which reads it in a way that works for a guest, unlike the command this pane used). "Load All" now finishes by reading the settings too, so it still leaves the whole repeater on screen, and each group opens itself once it has something to show. Where a value is shown in two places -- the name and position also live in Node Info, the owner note and guest password in Owner Info -- it is asked of the repeater once: the pane's answer fills the editor's field as well, and the settings read that follows skips it, five round trips fewer over the air. It works the other way too, so a name or owner note read or changed in the editor updates the pane beside it rather than leaving two different values on one screen. A read gives up early when nothing answers back: a guest session used to mean waiting out a ten-second timeout for every setting in the list, and now costs half a minute to establish that nobody is listening

* Bug: A picture sent from here arrived in other apps with "Node 68b8" where the sender's name belongs. A picture chunk carries two bytes of its sender's identity and nothing else, and the copy handed to an app connected to this node carries those two bytes flipped -- without that the app throws the picture away, because it uses this radio's identity as its own and every picture from here looks to it like its own echo coming back. Nothing in its contact list matched the flipped bytes, so it had nothing to print but the bytes. This node now also appears in that contact list under its own name, as the identity its pictures carry, so a photo arrives with the radio's name on it. That entry is this node wearing a mask rather than another node on the air, so a message addressed to it is refused instead of being transmitted to a key nobody holds

* Bug: Sending a picture to a channel could put a line reading `aeib:grp:` and sixteen characters of hex into everyone else's app, instead of the picture. That text is this server's own bookkeeping: a picture crosses the air as binary, so there is no message to keep, and the row that gives the photo its place in the conversation holds that marker instead. Two things then conspired. The radio hands back a copy of what it just transmitted, and while pictures decoded off the packet log had always been checked for "this is our own send, ignore it", pictures arriving by the other route were not -- so every photo sent from here also arrived here, as a second bubble for the same picture, and because it carries this node's own identity it was filed as one we had sent. That second bubble was an ordinary outgoing channel message as far as everything else was concerned, so it offered Copy text and a resend -- and a resend sends the *text*, which is the marker. It is now impossible from either end: the copy of our own picture coming back is ignored, and a picture's row is refused by the resend, by the automatic one-shot resend, and by the copy and retry buttons, which no longer appear on a picture. Deleting one, cancelling it and reacting to it still work

* Bug: A picture arriving on a channel put a line reading `aeib:grp:` and sixteen characters of hex on your phone, as the push notification for it. That text is this server's own bookkeeping -- a picture crosses the air as binary, so there is no message to keep, and the row that gives the picture its place in the conversation holds that marker instead; the web page it was written for shows you the photo. Only the connected-apps side ever knew to skip it, so everything else that reads a message as words passed it along verbatim: the notification on your phone, anything subscribed over MQTT, and every bot -- which for a bot that repeats what it hears is a route back onto the mesh. A photo now notifies as "📷 Photo" (and something this build cannot open as "📎 Media"), since knowing a picture arrived is the point of the notification, while MQTT and bots skip those rows entirely

* Bug: A picture sent from RemoteTerm never appeared in a connected app, and a picture sent from an app appeared in RemoteTerm with a string of hex where the sender's name goes. Both come from the same thing: an app using this server as its radio is told the radio's identity, so it believes the radio's public key is its own -- and a picture carries the first two bytes of its sender's key in every chunk. MeshCore Open Advanced throws away any picture chunk stamped with its own key, which is right on a real radio (the only way to hear your own is a repeater bouncing it back) and means that every picture this server handed an app was discarded before it was ever assembled. The copy handed to apps now carries a stand-in for those two bytes, so it arrives and is displayed; what goes on the air is unchanged, and other people's pictures are passed through untouched. Going the other way, a picture received on a channel is now attributed: one an app sent through this node is stored as something this node sent, exactly as a message from an app already was, and one from someone else is matched against the contact list and shown with their name. Before, it had no sender at all, and the conversation fell back to showing the channel's own key -- a shared secret, printed where the author should be

* Bug: An app connected to the virtual companion node could not send a picture, and could not send a compressed message either -- only plain text went out. Both travel as binary channel data rather than as text, and that command was passed straight to the radio with the app's own channel number in it. Apps address channels by a slot number that belongs to this server's table, which is not the radio's: the radio holds a handful of slots and re-uses them as it sends, so the picture was either refused outright -- "could not send" in the app and nothing anywhere else -- or encrypted for whatever channel happened to be sitting in that slot and transmitted perfectly, to nobody. The blob is now addressed to the right slot on the radio, exactly the way a channel message from an app already was, and goes out byte for byte as the app framed it. What an app sends this way now also appears in RemoteTerm: a compressed message becomes an ordinary message and a picture is stored and shown, instead of leaving no trace of whether it went at all. Compressed channel messages from other people's apps arrive as messages now too -- MCO Advanced sends them as binary by default, and every one of them used to be identified in a log line and dropped, so a channel where people had compression on looked quiet

* Bug: Pictures were invisible to apps connected to the virtual companion node -- both the ones this server sends and the ones it receives. An app using this server as its radio never hears what the radio transmits (it *is* the radio), and the picture frames the radio does hand over were not being passed on, so a photo sent from the web UI simply never arrived in the app, and one received from someone else showed up as a line of text reading `aeib:` and a number, which is this server's own private bookkeeping and means nothing to anyone else. Apps are now handed the picture itself in both directions -- including pictures decoded straight off the packet log, which is the only route on radios whose firmware never hands binary channel data to a companion at all -- addressed to the channel slot that app knows it by, and the same picture arriving by two routes is only shown once. Note that a picture sent as a *direct message* still goes out in RemoteTerm's own text form, which MeshCore Open Advanced cannot read: it has no direct-message picture transport at all, so there is nothing on the other end to read it with. On channels both ends now speak the same format

* Bug: A message sent from a connected app was reported as sent, went out on the air, was repeated and came back to this node -- and reached nobody. An app sets the radio's flood scope immediately before each message it sends, and the virtual node passed that straight through to the radio. Flood scope sticks: it stays in force until something changes it, and RemoteTerm only overrides and restores it around a send when that channel has been given an explicit scope of its own. So the app's scope shaped the message, stayed on the radio afterwards, and shaped RemoteTerm's own sends too -- the packet transmitted and was repeated exactly as it should be, scoped away from the people it was addressed to, which looks identical to a message that worked. Setting the flood scope and the path hash mode are now acknowledged and dropped rather than applied: a message from an app goes out under this server's scope, the same as one typed in the browser, and per-channel scopes stay where they are set, in Settings → Channels. Channel slot assignments are also kept for good now instead of being worked out afresh at each start: apps address a channel by slot number and cache it, and re-deriving them re-sorted the table whenever a channel had been added or removed, so an app's cached slot came back pointing at a different channel -- again transmitting normally, encrypted for a channel nobody in that conversation was listening to. The activity trace in Settings → Virtual Node now records what each send resolved to (which channel a slot meant, which contact a key prefix meant), because from the app's side a message delivered to the wrong place and one delivered correctly look the same

* Bug: A connected app could not send channel messages while "allow connected apps to change radio settings" was off -- which is the default. An app scopes each channel message by setting the radio's flood scope immediately before sending it, exactly as RemoteTerm's own channel send does, and that command was classed as radio configuration and refused; the app gave up on the send without ever transmitting, and reported only that it could not send. Setting the flood scope and the path hash mode are per-send parameters, not configuration, and now always go through. Adding a contact from a shared card moved to contact management for the same reason -- it is not configuring the radio. The switch now covers what it says on the label: the node's name, where it says it is, how it transmits, its PIN, telemetry modes, custom variables, its default flood scope, and signing. Read-only mode still refuses all of it

* Bug: An app connected to the virtual companion node could not send on the public channel. Apps address a channel by slot number and treat slot 0 as the public channel without ever reading it back, exactly as the firmware lays it out -- but the node handed out slots in key order, so slot 0 went to whichever channel sorted first, which on a default install is the `#remoteterm` channel. A message sent on Public therefore went to the wrong channel, or to a slot the node had nothing in and refused outright. The public channel now always holds slot 0, including after an app clears that slot, and the channel it displaces keeps a slot of its own. A direct message is now also answered with the radio's own send confirmation rather than one rebuilt from the pending-ACK table: it carries the delivery code and timeout the firmware actually chose, so an app moves the message from "sending" to "delivered" when the acknowledgement comes back instead of waiting on a code that never matches. Settings → Virtual Node now also lists the channel slot map and traces the last sixty commands -- what each connected app asked for, what it was answered with, and why when that was an error, filterable to failures. An app that will not send says only that, and the app is the one piece of this you cannot see into: whether it even sent the command, and what came back, used to exist nowhere but the server log

* Feature: Other MeshCore apps can now use the radio through RemoteTerm. A companion radio talks to one host, and RemoteTerm is that host -- so the phone app, meshcore-cli or Home Assistant had to be unplugged from it to be used at all. With `MESHCORE_VIRTUAL_NODE_ENABLED=true` RemoteTerm opens a TCP server speaking the companion protocol on port 5000, the port a WiFi companion listens on, and any of those apps connects to the server as if it were the radio; several can be connected at once, the way MeshMonitor's virtual node does it for Meshtastic. Most of what an app asks never reaches the radio: identity, the contact list (the whole server-side list, not the few hundred the radio can hold), channels, the clock, battery and every incoming message are answered from what RemoteTerm already knows, and read-only device queries are cached for thirty seconds so three apps polling cost one round trip. Sends, repeater logins, telemetry requests and radio settings are forwarded under the same lock RemoteTerm uses for its own commands, so an app can never interleave with a sync or a send in progress. A message sent from an app is stored, shown, ACK-tracked and retried exactly like one typed in the browser, and a contact or channel added in an app appears in RemoteTerm too. Rebooting, factory reset and private-key import are refused, private-key export follows the existing `MESHCORE_ENABLE_LOCAL_PRIVATE_KEY_EXPORT` switch, and `MESHCORE_VIRTUAL_NODE_READ_ONLY=true` turns connected apps into viewers. Like a real WiFi companion the port has no authentication of its own, so it stays off unless asked for; Settings → About shows whether it is listening, how many apps are connected and how much was answered locally versus forwarded. An app that reconnects picks up where it left off: the protocol names the app but not the device, so each app is remembered by that name plus the address it connects from, and on its next connection it is handed the incoming messages it missed, oldest first, up to `MESHCORE_VIRTUAL_NODE_REPLAY_LIMIT` (1000; the newest win when more were missed, and a first-time app starts at the present rather than being flooded with history). The Home Assistant add-on exposes the node's port under Network and gains options for enabling it, read-only mode and the replay limit. Changing the radio's own settings from an app -- name, frequency, TX power, flood scope and the like -- is off by default and refused until switched on, because a phone that can reach the port should not be able to retune the radio the whole server depends on; the switch lives in a new Settings → Virtual Node page, which also lists every app connected right now (with a Disconnect button) and every app the node remembers, with how far into the history each has caught up and a Forget button that makes its next connection start at the present

* Feature: Bots now work in room servers. A room post travels as a direct message from the room's own contact, so a bot saw one as a DM and answered the person who wrote it -- privately, off to one side, while the room that asked the question never saw the reply, and when the poster was only known by a key prefix the reply went nowhere at all. A room is now its own kind of conversation: `msg.is_room` says so, `msg.room_key` / `msg.room_name` say which one, and `ctx.reply` posts the answer back into the room where everyone logged in sees it. Bot code gains `ctx.send_room("Ops Board", text)` and `ctx.send_room_image(...)` for posting into any room by name or key, and the Test tab can simulate a room post alongside a DM and a channel message. Because the answer is now public, rooms get their own list on a bot's Settings tab -- all rooms, only the ones you pick, or all except the ones you pick, exactly like the channel list, so a bot can answer in the ops room and stay out of the noisy one. Rooms are opt-in: a new bot, and every bot you already have, starts with that list empty and answers in no room until you name one, because a room answer is public to everyone logged in rather than a DM to one person. A node that upgraded while an earlier build handed bots every room -- inherited from "Respond to direct messages" -- is taken back off "All rooms" here, since that build's upgrade step never runs twice; a bot you had deliberately pointed at every room needs its rooms picked again, and one pointed at a named list is untouched. A bot that already opted out of channels or DMs in code (`if not msg.is_dm: ...`) still opts out here, since a room is neither. Our own posts coming back around are ignored: a room relays every message to every member, including us, and reacting to that is how two bots end up answering each other forever

## [4.0.1] - 2026-08-26

* Change: The About panel credits both authors and shows the version this fork actually released. It said "Made with love and open source by Jack Kingsman" and offered one coffee link; it now credits Jack and Tristan and offers both. The version under the title said v3.17.1, which was upstream's last release -- it reads installed package metadata, so it followed pyproject.toml, and nothing here had ever bumped that because the script that does it only ever ran upstream. It now says 4.0.0, matching the changelog. The "unknown" beside it is separate and still there: the Dockerfile defaults the commit hash to that word and the Home Assistant build passes no real one

* Change: The project is now called RemoteTerm for MeshCore Advanced, and every link in it points at this fork rather than the repository it was forked from. The links that were wrong were the ones people actually follow: Report a Bug and Changelog in the About panel, the Home Assistant and HTTPS help links in settings, the room-server issue link, the clone command in the README, and the Documentation line in the systemd unit -- all of which led to a repository that has none of the features described. The prebuilt-frontend installer and the AUR PKGBUILD now fetch release assets and source tarballs from this fork too, so an install gets the code it was asked for. Nothing was renamed that anything depends on: the add-on slug, the AUR package name, the systemd unit, the Python package and every MESHCORE_ variable are unchanged, so an upgrade is an upgrade rather than a second installation. The original author's copyright and credit stay where they are, and the README now says what this is a fork of
...
* Change: `MESHCORE_ENABLE_AEIC=false` now switches off *rebuilding* AI photos rather than the whole codec, so a server set that way can still send them. The two halves never cost the same thing: rebuilding a photo someone sends you needs 893 MB of model and about 1.4 GB of memory every time, which is what anyone turning this off is turning off, while sending needs 65 MB and touches none of it. Photos that arrive are still kept and shown as a box, so switching it back on decodes them. The settings panel now says exactly that, instead of reporting the codec as switched off and telling you to install a dependency that is already installed. A fresh install with it set to false still gets nothing at all: the codec's dependencies are only installed when it is on, and what false cannot do is remove what turning it on once already put there

* Feature: Sending AI (AEIC) photos no longer waits on a 958 MB download. The model has two halves and only one of them is needed to send: 65 MB, which costs no memory worth worrying about, against 893 MB and about 1.4 GB per picture for rebuilding what other people send. The sending half is now fetched on its own as soon as the codec is switched on -- so a server can send AI photos without anyone choosing to download the rest, and a small Pi that cannot rebuild a picture at all can still send them. Rebuilding stays an explicit choice, offered as "download the rest" once sending works. A server that had no internet when it started tries again the first time you send, rather than waiting for a restart, and the progress bar now measures whichever half is being fetched instead of showing a finished 65 MB download as 3% of the whole bundle. An explicit MESHCORE_ENABLE_AEIC=false still downloads nothing at all

* Bug: Receiving an AI (AEIC) picture could kill the whole server -- not the picture, the process, radio link included, with nothing in the log to say why. Reconstruction is by far the largest thing this app ever allocates, and when a host does not have the room the kernel's out-of-memory killer picks the biggest process on it, which was RemoteTerm itself. Reconstruction now happens in a short-lived worker process, so a host that cannot fit one loses the worker and gets told the picture needs more memory than the machine has -- and the picture is kept either way, so it can be opened later or from somewhere roomier. Two things follow for free: the memory is fully returned to the system after every picture instead of being held for the life of the server, and a server that only sends pictures never loads the big model at all

* Change: An AI (AEIC) picture now reconstructs in about 1.4 GB of memory instead of about 2 GB, which is the difference between working and being killed on a small Pi or VPS. The model's 832 MB of weights are now read straight from disk rather than copied into memory first, so the system can reclaim them under pressure instead of running out; measured end to end against hard memory limits on a real picture, reconstruction that was killed outright at 1.75 GB now completes at 1.4 GB and produces a byte-identical image. Below about 900 MB free the codec now says so plainly -- naming what it needs and what the machine has -- rather than attempting it. That figure is read from the container's own limit where there is one, since inside a container the free memory of the host says nothing about what this app may use. Sending is unaffected and stays available on hosts too small to receive: encoding never touches the large half of the model

* Bug: On a phone, the Conversation features panel ran off the top and bottom of the screen with no way to scroll it: the heading and its close button sat above the top edge, the last setting was cut off below the bottom one, and tapping outside the panel was the only way out. Dialogs are centred on the screen, so anything taller than it overflows in both directions at once -- and nothing capped their height. Every dialog now fits the screen it opens on and scrolls what does not fit; in the features panel only the list of settings scrolls, so the heading and the close button stay where they are however many features are on. Dialogs that already managed their own scrolling are unchanged

* Bug: A picture sent to a channel from MCO Advanced could fail to appear at all, even though the radio heard it. Binary channel data reaches the app only when the firmware hands it over on request, and some firmware never does -- the packet arrived, was logged in the packet feed, and went nowhere. Channel pictures are now decoded straight off the received-packet log, the same way channel text always has been: it works on any firmware whose radio can hear the packet, needs no channel loaded on the radio, and cannot decode wrongly -- every packet's authentication code is checked first, so a wrong key produces nothing rather than a wrong picture. The layout of what is inside was taken from the radio firmware's own source, and the change was proven against a real captured packet: a photo sent from MCO Advanced to #bots that had never appeared, which decodes to exactly what the sender's app reported. Echoes of this node's own pictures, re-flooded by repeaters, are recognised and not shown a second time. The firmware hand-over still works as a fallback where the packet log is unavailable, and a picture arriving by both routes still appears once

* Bug: One picture arriving through the radio's message queue silently killed message fetching. The frame carrying it is one the message-fetch machinery does not recognise, so the fetch that pulled it waited forever for an answer that had already arrived -- and while that fetch hung, no new one could start, so every message the radio announced afterwards sat in its queue unfetched. The picture frame now releases the fetch that pulled it and fetching continues to the next message; queue drains no longer stop early at a picture, and the periodic audit no longer counts one as a message the app missed

* Bug: Sending the same photo to a channel again never showed it a second time. A received channel picture is stored under a name derived from the picture itself, so one photo is only ever stored and decoded once — but that name was also what identified the message, and the app treats two channel messages with the same text as the same message. So every resend of a photo landed on the first one's row and vanished: no bubble, nothing pushed to the conversation, and no way to tell it apart from the picture never arriving. Resending is the first thing anyone does when a picture does not turn up, which made the obvious way to check the one way guaranteed to fail. Each send now gets its own bubble, while the picture behind them is still stored and decoded once. A copy re-flooded by a repeater is still recognised as the same picture and does not add a second bubble

* Feature: Pictures and voice notes now fetch as ordinary messages, which means they work on nodes whose firmware cannot send raw data packets at all. The standard formats move their fragments with `CMD_SEND_RAW_DATA`, and firmware without it could not move a single byte in either direction — tapping a received picture answered "this node's firmware cannot send raw data packets" and there was nothing the person tapping it could do. Asking for the fragments now uses the same messages everything else travels on, so it works everywhere, at the cost of roughly 2.5x the airtime: one picture fragment is two messages, so a 20-fragment picture is forty of them and takes minutes rather than seconds. It is the default because a node that *can* send raw data loses only airtime this way, while a node that cannot gets a picture it could otherwise never open — and there is no way to tell which kind the far end is before asking. Turn it off per contact under Conversation features to spend raw packets and get a plain error where they are unsupported. Someone using another MeshCore app sees a run of short `rmt1:` lines in the conversation instead of a picture, which is the honest signal that their app cannot join in — it does not stop *them* asking for your picture, because a request that arrives as raw packets is always answered with raw packets

* Bug: A picture that only partly arrived reported itself as simply "Unavailable", and the app gave up waiting after 30 seconds no matter how large the transfer was — so a picture whose fragments were still coming in was written off as broken, and the fragments already received were hidden rather than counted. The app now waits for as long as parts keep arriving and only stops when they genuinely stop, and an incomplete picture says how much of it is missing and offers to ask for the rest. Retrying asks the sender only for the parts that never came, so it costs a fraction of a fresh transfer instead of starting over. Voice notes report and retry the same way

* Feature: Node stats page — clicking a repeater's clock drift now opens a page of its own for that node, with its own time range from the last hour out to everything stored. It carries the full drift history as a line with the spread of readings shaded around it, the moments the clock was *set* rather than drifting (a reboot, a manual sync, a GPS fix landing) with the time between readings noted so you can judge how precisely each one is dated, a breakdown of readings by how many hops they arrived over — which shows how much of the offset is really mesh airtime rather than the clock — and the spread of that node's own readings. Reachable from the drift section's History link, a View node stats row in the contact pane, or by clicking any repeater in the Statistics drift tables, and linkable directly. The page is built to hold more than drift: other per-node statistics will appear on it as they are added

* Bug: A clock that keeps being reset was reported as holding steady. The trend was fitted across the resets, which averages the sawtooth away — one repeater losing ten minutes a day between reboots read as barely moving, and the advice that followed said one resync would fix it. The trend is now measured over the stretch since the clock was last set, and says so when it is; where resets have happened they lead the summary, because a clock that will not hold its time is a different repair from one that was set wrong once. This corrects the figure in the contact pane as well, not only on the new page
* Bug: Opening a picture or voice message from a contact more than three hops away failed with "raw media transfer is limited to 3 routed hops". That was never a mesh rule — ordinary messages already travel those routes, and asking the sender for fragments is a single small packet on the same path. The real limits are the packet header's 6-bit hop field and MeshCore's 64-byte path, so up to 63 hops with single-byte path hashes (32 or 21 with the wider ones), matching what the phone apps accept. The refusal also names the hop count it rejected now, so a nonsense learned path shows up instead of hiding behind a generic limit
...
* Bug: Voice messages sent from here, copied elsewhere and sent back still refused to play, with "voice session ID describes a different recording". The VE3 envelope carries the length as a whole number of seconds, rounded up, while a send stores the exact length of the recording — so a 3.5 second clip was stored as 3457 ms and read back off its own envelope as 4000 ms, and the check meant to catch two genuinely different recordings on one id rejected nearly every re-sent one instead. Lengths are now compared at the resolution the envelope actually carries. Recordings a whole second or more apart are still correctly refused
