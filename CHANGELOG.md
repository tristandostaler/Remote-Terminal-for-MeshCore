...
* Bug: A reaction that lands on no message now says why in the log, instead of leaving you to guess. A reaction names its target by a four-character hash and nothing else, so when it matches nothing there is no message id to chase and no way to run the hash backwards -- but the ways two clients can disagree about one message are few, and each is a rule the hash can be recomputed under. RemoteTerm now tries them: the timestamp shifted second by second a quarter hour either way and by whole hours (a peer stamping its messages in local time rather than UTC), the sender-name rule it did not use (the two ends disagreeing about whether a conversation is a room server, where the author's name goes into the hash, or a one-to-one chat, where it does not), and the text normalisations either end may have applied -- surrounding whitespace, a trailing null byte off the radio's own frame, a stripped reply mention. The first one that would have matched is logged as exactly that, naming the message and the difference, so a clock problem and a text problem are told apart without reading a single hash. None of it is ever accepted as a match: sixteen bits tried under thousands of variations starts colliding, and a reaction attached to the wrong message is worse than one attached to nothing. The emoji table's hashes are also now generated by running MeshCore Open Advanced's own code on the Dart runtime rather than transcribed from it -- all thirty-three existing values came back identical, which rules the hash itself out of any future investigation
