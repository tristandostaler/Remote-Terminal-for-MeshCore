...
* Change: The `mailbox` bot answers to the `mbx` keyword instead of watching every message for its own prefix, so it now shows up in the `help` command list like every other command and can be given extra trigger words on its Triggers tab — whichever word you type is the one its replies quote back at you. Its "The mailbox command prefix" setting is gone (the trigger *is* the prefix), and what still runs on every message is only the passive name → public-key learning that lets `mbx to <name>` address a node this companion has heard
